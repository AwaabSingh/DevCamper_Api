...
exports.getCourse = asyncHandler ( async (req, res,next) => {
    ...
    if(!course) {
        return next(new ErrorResponse(`No course the id of ${req.params.id}`),404)
    }
    res.status(200).json({
        success: true,
        data: course 
...
exports.addCourse = asyncHandler ( async (req, res,next) => {
    // get the bootcamp id and submit as a body field
   req.body.bootcamp =  req.params.bootcampId
   // get the usrer id and submit as a body field
   req.body.user = req.user.id

   // check if the bootcamp exsit
   const bootcamp = await Bootcamp.findById(req.params.bootcampId)

if(!bootcamp) {
    return next(new ErrorResponse(`No bootcamp the id of ${req.params.bootcampId}`),404)
}
